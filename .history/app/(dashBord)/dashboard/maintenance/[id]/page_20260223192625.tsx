...
const isAssignedToMe = item?.assignedUser?.id === (session?.user as any)?.id;
    ...
    {item.assignedUser ? (
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-gray-400">
            {isAssignedToMe ? "You are responsible for this job" : "Contact superuser to reassign"}
          </p>
        </div>
      </div>
    ) : (
      ...
      {/* Actions — only show if superuser OR assigned to current user */}
{!isClosed && (isSuperuser || isAssignedToMe) && (
  // ... existing actions block stays the same
)}
