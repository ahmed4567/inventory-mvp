...
type MaintenanceItem = Awaited<ReturnType<typeof getMaintenance>>;
...
export default function MaintenanceDetailPage({
  ...
  const [item, setItem]             = useState<MaintenanceItem>(null);
  ...
  const isAssignedToMe = item?.assignedUser?.id === currentUserId;
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      {/* Assignment — superuser only */}
      {isSuperuser && !isClosed && (
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 mb-5">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-blue-100 flex items-center justify-center
                  ...
                  {(item.assignedUser.name ?? item.assignedUser.email)[0].toUpperCase()}
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-800">
                    {item.assignedUser.name ?? item.assignedUser.email}
                  </p>
                  <p className="text-xs text-gray-400">{item.assignedUser.email}</p>
                </div>
              </div>
              ...
              <div className="flex gap-2">
                <select
                  onChange={async (e) => {
                    ...
                    if (!val) return;
                    await assignMaintenance(id, val === "unassign" ? null : val);
                    setAssigning(false);
                    refresh();
                  }}
      ...
      {/* Assigned info for normal users */}
      {!isSuperuser && (
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 mb-5">
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-2">
            Assigned To
              ...
              <div>
                ...
                <p className="text-xs text-gray-400">
                  {isAssignedToMe
                    ...
                    : "Contact superuser to reassign"}
                </p>
              </div>
            </div>
          ) : (
            <p className="text-sm text-gray-400">Not assigned — contact your superuser</p>
          )}
        </div>
      )}

      {/* Actions — superuser or assigned user only */}
      {!isClosed && (isSuperuser || isAssignedToMe) && (
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 mb-5">
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-4">
            Update Status
          </p>
          <div className="flex flex-wrap gap-3">
            ...
            {isSuperuser && (
              <button onClick={() => {
                ...
                className="border border-red-300 text-red-600 px-5 py-2 rounded-lg
                           hover:bg-red-50 font-medium disabled:opacity-50">
                Cancel Job
              </button>
            )}
          </div>
      ...
      {/* Message for non-assigned normal users */}
      {!isClosed && !isSuperuser && !isAssignedToMe && (
        <div className="bg-gray-50 rounded-xl border border-gray-200 p-5 mb-5 text-center">
          <p className="text-sm text-gray-500">
            You are not assigned to this job.
            Only the assigned technician or a superuser can update its status.
          </p>
        </div>
      )}
