...
export async function registerUser(data: unknown) {
  ...
  if (existingEmail) return { error: { email: ["Email already registered"] } };

  const existingUsername = await prisma.user.findUnique({
    where: { username: validated.data.username },
  });
  if (existingUsername) return { error: { username: ["Username already taken"] } };
...
export async function createUser(data: unknown) {
  ...
  if (existingEmail) return { error: { email: ["Email already exists"] } };
